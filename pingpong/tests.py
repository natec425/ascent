...
class TestUserSeesLeaderboard(TestCase):
    def test_successfully(self):
        winner = User.objects.create(username="winnie", id=1)
        loser = User.objects.create(username="lucy", id=3)
        matches = [
            Match.objects.create(player1=winner, player2=loser, player2_score=7, player1_score=11)
        ]

        response = self.client.get(reverse("pingpong:home"))

        self.assertContains(response, "Leaderboard")
        self.assertContains(response, "winnie")
